...
def load_img_folder(img_folder_path):
    ...
    for img  in glob.glob(img_folder_path): # "./img/*.jpg"
...
#    for i in range(len(file_names)):   # lenght = 4320
#            if csv_names[x] in file_names[i]:
#                print(csv_names[x], x, "gefunden in item", file_names[i], i) # Bildname aus .csv gefunden in Ordner
#                print(csv_data[x])
    ...
    for i in range(len(csv_names)):   # lenght = 4320
        ...
        if image_name in csv_names[i]:
            #print(image_name, "gefunden in item", csv_names[i], i) # Bildname aus Ordner gefunden in .csv
            #print(csv_data[i])
...
#truth_box = [np.array([633, 383, 845, 578]), np.array([205, 383, 480, 591])]
